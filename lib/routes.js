...
/**
 * Homebrew prefix prepended for CLI children: the desktop-launched host has
 * a minimal PATH that lacks pnpm/corepack/node, so pnpm installs fail.
 */
const CLI_PATH = '/opt/homebrew/bin:/usr/local/bin';
function childEnv(extra) {
    return { ...process.env, ...(extra ?? {}), PATH: `${CLI_PATH}:${process.env.PATH ?? ''}` };
}
