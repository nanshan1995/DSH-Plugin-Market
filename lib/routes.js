...
function probePnpm() {
    ...
    return new Promise((resolvePromise) => {
        ...
        const child = spawn(pnpm ?? 'pnpm', ['--version'], { env: childEnv(), stdio: 'ignore', shell: WIN32 });
...
function runQuiet(file, args, timeoutMs) {
    return new Promise((resolvePromise) => {
        const child = spawn(file, args, { env: childEnv({ CI: 'true' }), stdio: ['ignore', 'pipe', 'pipe'], shell: WIN32 });
        let output = '';
...
function extractTarball(tarball, dest) {
    return new Promise((resolvePromise, rejectPromise) => {
        const child = spawn('tar', ['xzf', tarball, '-C', dest], { stdio: ['ignore', 'pipe', 'pipe'], shell: WIN32 });
        let stderr = '';
...
export async function searchPlugins(query, lang = 'en', limit = 12, page = 1, sort = 'hot', extraTerms = []) {
    ...
    if (hit !== undefined && Date.now() - hit.at < GH_SEARCH_TTL_MS) {
        ...
        // Hot (default) ranks by stars so well-known curated/community repos
        // surface instead of GitHub's pure relevance order burying them;
        // `new` ranks by updated. Browse keeps plain topic listing.
    ...
    const combined = p === 1
        ...
        : enriched;
    // Hot (default) ranks by stars — curated and community together, so the
    // well-known curated entries surface rather than being buried under
    // GitHub's relevance order; `new` ranks by updated. Keyword search with
    // hot sorting therefore interleaves curated + community by popularity.
    if (sortNew) {
        combined.sort((a, b) => String(b.pushed ?? b.added ?? '').localeCompare(String(a.pushed ?? a.added ?? '')));
    }
    else {
        ...
        for (const dir of dirs) {
            ...
            for (const file of ordered) {
                ...
                try {
                    ...
                    return { content, repaired, source: file, dir };
                }
                catch { /* keep looking */ }
            }
            ...
            if (anyReadme !== undefined) {
                try {
                    ...
                    return { content, repaired, source: anyReadme, dir };
                }
                catch { /* keep looking */ }
            }
        }
    ...
    const disposers = [
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    // force=1 → 跳过内存缓存，实时拉取精选目录（失败仍回退缓存/快照）。
