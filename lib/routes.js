...
import { existsSync, readFileSync, rmSync, mkdtempSync, lstatSync, writeFileSync } from 'node:fs';
...
/**
 * Extra PATH prefixes prepended for CLI children: the desktop-launched host
 * may run with a minimal PATH that lacks pnpm/corepack/node. Platform
 * specific (Homebrew on macOS, /usr/local on Linux; nothing extra on win32).
 */
const CLI_PATH_EXTRA = process.platform === 'darwin'
...
function childEnv(extra) {
    ...
    if (CLI_PATH_EXTRA !== '') env.PATH = `${CLI_PATH_EXTRA}:${env.PATH ?? ''}`;
    return env;
}
...
function probePnpm() {
    ...
    return new Promise((resolvePromise) => {
        const child = spawn('pnpm', ['--version'], { env: childEnv(), stdio: 'ignore' });
...
export { readTimesLedger, writeTimesLedger, collectTimes };
    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: (request, response) => {
                ...
                // Merge patch-loaded community rows (not package.json deps)
                // into the management view, so every third-party plugin is
                // toggleable regardless of how it was installed. Internal
                // rows (official scope, cordis: machinery) stay out.
                    ...
                    if (spec === undefined && readLoaderEntries().some((row) => row.name === name && !row.name.startsWith('@deepseek-ai/') && !row.name.startsWith('cordis:'))) {
                        ...
                        try {
                            ...
                            for (const entry of list) {
                                ...
                                if (entry.id === row.id && Object.keys(entry).every((k) => k === 'id' || k === 'disabled')) continue;
                                if (Array.isArray(entry.insert)) {
                                    const insert = entry.insert.filter((r) => r && typeof r === 'object' && r.id !== row.id);
