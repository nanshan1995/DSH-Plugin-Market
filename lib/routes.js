...
import { exportLogs, logEvent, sanitize } from './log.js';
...
/**
 * GitHub raw 拉取通道：优先 curl 子进程。Node 内置 fetch 不读代理环境变量，
 * 在需要代理的网络（如国内直连被墙）上会反复超时/失败；curl 继承 shell 的
 * HTTPS_PROXY（Clash 7897 等）稳定可达。Windows 10+ 自带 curl.exe。
 * 失败时回退 undici fetch（直连尝试）。
 */
const CURL_BIN = process.platform === 'win32' ? 'curl.exe' : 'curl';
// GUI 启动的进程不继承 shell env，用常见本地代理端口兜底探测。
const LOCAL_PROXY_PORTS = [7897, 7890, 10809, 10808, 1080, 8888, 8118, 8080];

function envProxyUrl() {
    const e = process.env;
    for (const k of ['DSHMARKET_README_PROXY', 'HTTPS_PROXY', 'https_proxy', 'HTTP_PROXY', 'http_proxy', 'ALL_PROXY', 'all_proxy']) {
        const v = e[k];
        if (typeof v === 'string' && v.trim() !== '' && !/^socks/i.test(v.trim())) return v.trim();
    }
    return null;
}

function probeProxyPort(port, timeoutMs = 1200) {
    return new Promise((resolve) => {
        ...
        sock.on('connect', () => {
            sock.write('CONNECT raw.githubusercontent.com:443 HTTP/1.1\r\nHost: raw.githubusercontent.com:443\r\n\r\n');
        });
        let buf = '';
        sock.on('data', (d) => {
...
function detectProxyUrl() {
    if (proxyUrlPromise === null) {
        proxyUrlPromise = (async () => {
            const envUrl = envProxyUrl();
            if (envUrl !== null) return envUrl;
            // 并行探测全部候选端口：无代理的机器最多等一个探测超时
            // （~1.2s）而不是逐个串行累计（~10s），然后回落直连。
            const hits = await Promise.all(
                LOCAL_PROXY_PORTS.map(async (port) => ((await probeProxyPort(port)) ? port : null)),
            );
            const port = hits.find((p) => p !== null);
            return port !== undefined && port !== null ? `http://127.0.0.1:${port}` : null;
        })();
    }
...
function runCurl(args, timeoutMs) {
    return new Promise((resolve) => {
        execFile(CURL_BIN, args, { timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024, windowsHide: true, encoding: 'buffer' },
            (error, stdout) => {
                if (error || stdout === undefined) resolve(null);
                else resolve({ buf: stdout });
            });
    });
}

/** 拉取文本（README 用），保留原始字节供乱码修复；失败返回 null。 */
async function curlText(url, timeoutMs) {
    const base = ['-sS', '-f', '-L', '--max-time', String(Math.max(5, Math.floor(timeoutMs / 1000))), '-A', 'dsh-plugin-market-readme'];
    const proxy = await detectProxyUrl();
    const attemptList = proxy !== null
        ? [[...base, '-x', proxy, url], [...base, url]] // 代理失败时直连重试一次
        : [[...base, url]];
    for (const args of attemptList) {
        const got = await runCurl(args, timeoutMs);
...
function dshArgv() {
    ...
    if (entry !== undefined && /[\\/](?:bin[\\/]dsh\.(?:js|cjs|ts)|bin\.(?:js|ts)|dsh)$/.test(entry)) {
...
function spawnTool(file, args, opts = {}) {
    if (WIN32 && opts.shell !== false) {
        // cmd.exe 的双引号转义是 ""（不是 \"）；参数包在双引号内时
        // & | < > 等元字符不会被 cmd 解析，无需 ^ 转义。
        const quote = (s) => (/\s/.test(s) ? `"${s.replace(/"/g, '""')}"` : s);
        const cmdline = [quote(file), ...args.map(quote)].join(' ');
        return spawn('cmd.exe', ['/d', '/s', '/c', cmdline], { ...opts, shell: false, windowsHide: true });
    }
    return spawn(file, args, { windowsHide: true, ...opts });
}

/** 超时/失败时杀掉整个子进程树：Windows 上 child.kill 只杀 cmd 壳，
 *  用 taskkill /T /F 连 pnpm/npm/dsh 的 node 子进程一起杀，避免孤儿
 *  进程占用 pnpm lockfile。 */
...
function readInstalled(profile) {
    try {
        ...
        for (const [name, spec] of Object.entries(manifest.dependencies ?? {})) {
            // 官方内置依赖：@deepseek-ai/ 前缀且按版本号来源安装。
            // 从 github:/link:/file: 装来的包即使包名带 @deepseek-ai/
            // 前缀（如 @deepseek-ai/dsh-client-ui-aqua）也是社区插件，计入管理。
            const communitySource = typeof spec === 'string' && /^(github:|git\+|link:|file:|workspace:)/.test(spec);
            if (name.startsWith('@deepseek-ai/') && !communitySource) continue;
            installed[name] = spec;
        }
        return installed;
    }
 ...
 */
/** 安全写盘守卫：patch 里含 !!js 等自定义 YAML tag 时（dsh 允许），
 *  load→dump 往返会丢失表达式语义、可能让下次启动失败——直接放弃写盘，
 *  宁可清理不干净也不写坏。 */
function patchTextSafeToRewrite(text) {
    return !/!![A-Za-z0-9_-]+|!<[^>]+>/.test(text);
}

/** 写盘前备份当前 patch，出问题可手动恢复。 */
function backupPatchFile(file) {
    try {
        writeFileSync(`${file}.bak-${Date.now()}`, readFileSync(file, 'utf8'));
    }
    catch { /* best effort */ }
}

async function cleanProfilePatch(profile, name, rowIds, names) {
    ...
    if (!patchTextSafeToRewrite(text)) {
        logEvent('warn', 'uninstall', 'cordis.patch.yml contains custom YAML tags — left untouched');
        return;
    }
    let yaml;
    try {
    ...
    for (const entry of list) {
        ...
        if (Array.isArray(entry.insert)) {
            const insert = entry.insert.filter((row) => {
                ...
                // 匹配被卸载包声明的装配 id/name（从它的 cordis.patch.yml 收集），
                // 以及包名本身——历史遗留行（如 usage-status）也一并清掉。
                const drop = (typeof row.id === 'string' && ids.has(row.id))
                    || (typeof row.name === 'string' && (nameSet.has(row.name) || row.name === name));
...
/** curl 下载到文件（-o），返回是否成功。用于审计 tarball 等大文件。 */
function runCurlToFile(args, dest, timeoutMs) {
    return new Promise((resolvePromise) => {
        execFile(CURL_BIN, [...args, '-o', dest], { timeout: timeoutMs, windowsHide: true },
            (error) => {
                resolvePromise(error === null || error === undefined);
            });
    });
}

async function fetchTarball(url, dest) {
    // 优先 curl 通道（自动探测代理：env → 本地端口 → 直连回退），
    // 直连 codeload/registry 在需要代理的网络会超时导致「审计不可用」。
    const base = ['-sS', '-f', '-L', '--max-time', '120', '-A', 'dsh-plugin-market-audit-gate'];
    const proxy = await detectProxyUrl();
    const attemptList = proxy !== null
        ? [[...base, '-x', proxy, url], [...base, url]]
        : [[...base, url]];
    for (const args of attemptList) {
        const ok = await runCurlToFile(args, dest, 130 * 1000);
        if (ok) {
            let size = 0;
            try { size = lstatSync(dest).size; }
            catch { /* fall through */ }
            if (size > AUDIT_SOURCE_LIMIT_BYTES) throw new Error('source tarball exceeds 64 MiB');
            return;
        }
    }
    throw new Error('tarball download failed');
}
function extractTarball(tarball, dest) {
    return new Promise((resolvePromise, rejectPromise) => {
        ...
        const child = spawn(file, ['-xzf', tarball, '-C', dest], { stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
        let stderr = '';
...
// README 拉取缓存（按 repo+lang / repo+path）：纯内存 Map，不写任何本地
// 文件；条目 30 分钟后自动过期释放（读取时惰性检查，无需定时器），
// 同时设容量上限（LRU：超出淘汰最久未用的条目），防长时间使用内存膨胀。
const README_CACHE = new Map();
const README_CACHE_TTL_MS = 30 * 60 * 1000;
const README_CACHE_MAX = 100;

/** 读缓存：过期即删并视为未命中；命中则刷新 LRU 位置。 */
function readmeCacheGet(key) {
    const hit = README_CACHE.get(key);
    if (hit === undefined) return undefined;
    if (Date.now() - hit.at > README_CACHE_TTL_MS) {
        README_CACHE.delete(key);
        return undefined;
    }
    README_CACHE.delete(key);
    README_CACHE.set(key, hit);
    return hit.data;
}

/** 写缓存：容量超限时淘汰最久未使用的条目（Map 首元素）。 */
function readmeCacheSet(key, data) {
    if (README_CACHE.has(key)) README_CACHE.delete(key);
    README_CACHE.set(key, { at: Date.now(), data });
    while (README_CACHE.size > README_CACHE_MAX) {
        README_CACHE.delete(README_CACHE.keys().next().value);
    }
}
...
export function mountMarketRoutes(host, config, llm, loader) {
    ...
    /** 本会话已卸载的包名：loader 里仍残留的 entry（bundle 层装配）不再
     *  计入管理视图——进程内即时「消失」，重启后依赖已删也不会再装配。 */
    const uninstalledNames = new Set();

    /** 收集一个已安装包在 cordis.patch.yml 里声明的装配 id/name 集合。
     *  必须在 dsh remove 删除 node_modules 之前调用。历史遗留的 profile patch
     *  insert 行（如 usage-status）靠这些引用在卸载时被精准清掉，否则重启时
     *  include 会尝试加载已删除的包导致 harness 启动失败。 */
    function collectBundleRefs(pkgName) {
        const ids = new Set();
        const names = new Set();
        for (const dir of [join(profileDir(config.profile), 'node_modules', pkgName), join(profileDir(config.profile), '..', 'node_modules', pkgName)]) {
            let text = '';
            try {
                text = readFileSync(join(dir, 'cordis.patch.yml'), 'utf8');
            }
            catch { continue; }
            for (const line of text.split('\n')) {
                const id = /^\s*-\s*id:\s*['"]?([A-Za-z0-9_.-]+)['"]?/.exec(line);
                if (id !== null) ids.add(id[1]);
                const nm = /^\s*name:\s*['"]?([^'"\s]+)['"]?/.exec(line);
                if (nm !== null) names.add(nm[1]);
            }
        }
        return { ids, names };
    }

    /** 从 profile 顶层 dsh.profile.bundles 数组移除包名（防重启后复活装配）。 */
    function dropBundleEntry(profile, name) {
        try {
            const file = join(profileDir(profile), 'package.json');
            const pkg = JSON.parse(readFileSync(file, 'utf8'));
            const bundles = pkg?.dsh?.profile?.bundles;
            if (!Array.isArray(bundles) || !bundles.includes(name)) return;
            ...
            logEvent('info', 'uninstall', `${name}: removed from profile bundles`);
        }
        catch { /* manifest unavailable — nothing to drop */ }
    }
    ...
    function mergedInstalled() {
        ...
        for (const row of readLoaderEntries()) {
            ...
            if (uninstalledNames.has(row.name)) continue;
    ...
    /** README 的实际语言：文件名含 zh/中文 → 中文；否则按内容 CJK 占比判断
     *  （有的仓库把中文 README 命名为 README.md，如 agent-vision-toolkit）。 */
    function readmeLangOf(file, text) {
        if (/(^|[\W_])zh([\W_]|$)|中文/i.test(file)) return 'zh';
        if (typeof text === 'string' && text.length > 0) {
            ...
            if (cjk / Math.max(1, sample.length) > 0.2) return 'zh';
        }
        return 'en';
    }

    /**
     * 把 README 里的相对路径图片/链接重写为 GitHub raw 绝对地址：
     * - `![alt](assets/x.png)` → `https://raw.githubusercontent.com/{repo}/HEAD/assets/x.png`
     * - `[text](docs/x.md)` → 同上（文档链接）
     * - `#锚点` 保留为 GitHub 页面锚点
     * - `dir` 为 README 所在目录（子目录 README 的相对路径按该目录解析，
     *   如 docs/lang/README_ZH.md 里的 `../x.png` → 仓库根 x.png）
     * 绝对 URL、mailto、协议相对（//）不动。
     */
    function rewriteReadmeUrls(content, repo, dir) {
        const d = dir && dir !== '' ? dir.replace(/^\/+|\/+$/g, '') + '/' : '';
        const base = `https://raw.githubusercontent.com/${repo}/HEAD/${d}`;
        const ghPage = `https://github.com/${repo}/blob/HEAD/${d}`;
        // 把 a/../b、./c 规范化为仓库内相对路径（不带开头 /）。
        const norm = (p) => {
            const stack = [];
        ...
        let out = String(content).replace(/(!?\[[^\]]*\]\()([^)]+)(\))/g, (whole, pre, target, post) => {
            const t = target.trim();
            if (t === '' || /^(https?:|mailto:|tel:|data:|#|\/\/)/.test(t)) return whole;
            // 相对路径：图片用 raw，普通链接用 GitHub blob 页
            const isImg = pre.startsWith('![');
            const path = norm(d + t.replace(/^\/+/, ''));
            return `${pre}${isImg ? base : ghPage}${path}${post}`;
        });
        // HTML 形式 <img src="..."> 的相对路径同样重写为 raw 地址。
        out = out.replace(/(<img[^>]*\bsrc=["'])([^"' ]+)(["'])/gi, (whole, pre, src, post) => {
            if (/^(https?:|data:|#|\/\/)/i.test(src)) return whole;
            return `${pre}${base}${norm(d + src.replace(/^\.\//, '').replace(/^\/+/, ''))}${post}`;
        });
        return out;
    }
        ...
        for (const dir of dirs) {
            ...
            for (const file of ordered) {
                ...
                try {
                    ...
                    return { content, repaired, contentLang: readmeLangOf(file, content), source: file, dir };
                }
                catch { /* keep looking */ }
            }
            ...
            if (anyReadme !== undefined) {
                try {
                    ...
                    return { content, repaired, contentLang: readmeLangOf(anyReadme, content), source: anyReadme, dir };
                }
                catch { /* keep looking */ }
            }
        }
    ...
    async function persistToggle(rowId, disabled) {
        ...
        try {
            ...
            if (!patchTextSafeToRewrite(text)) {
                logEvent('warn', 'toggle-persist', `cordis.patch.yml contains custom YAML tags — toggle not persisted for ${rowId}`);
                return;
            }
    ...
    // 路由级兜底守卫：async handler 的意外异常不悬挂响应，
    // 统一回 500 并写脱敏宿主日志。
    const guardedHandler = (inner) => async (request, response) => {
        try {
            await inner(request, response);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            try { host.logger?.warn(`[dsh-plugin-market] route error: ${sanitize(message)}`); } catch { /* logger gone */ }
            try { sendJson(response, 500, { error: 'route error' }); } catch { /* socket closed */ }
        }
    };
    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dsh-plugin-market] toggle failed: ${sanitize(message)}`);
        ...
        host.webServer.register({
            ...
            handler: guardedHandler(async (request, response) => {
                if (request.method !== 'GET') {
                    response.writeHead(405, { allow: 'GET' });
                    response.end();
                    return;
                ...
                // —— 通过 GitHub 在线拉取 README（「使用说明链接返回回来的」内容），
                // 失败返回 null。候选按语言优先级精简为 2 个（绝大多数仓库只用
                // README.md / README.zh.md）。5 路并发会互相挤占连接，raw 往返 ~5s
                // （TLS 握手主导）下随机超时，导致 zh 请求错拿英文、en 错拿中文；
                // 并发 2 竞争小，常见情况 5-6s 内拿到正确文件。404/403 是确定结果
                // 不重试；仅网络错误（超时/连接失败/5xx）重试一次。按候选顺序取
                // 「第一个成功」——用 allSettled 等全部结束后按序选，避免
                // Promise.any 取先完成的导致 zh/en 错位。语言不符时跟随 README
                // 内的语言切换链接（如 [**中文**](docs/lang/README_ZH.md)）。
                const fetchRemoteReadme = async (repo, wantLang, explicitPath, fresh) => {
                    const cacheKey = `${repo}\u0000${wantLang}`;
                    if (fresh !== true) {
                        const cachedReadme = readmeCacheGet(cacheKey);
                    ...
                    const fetchReadmeFile = async (file) => {
                        let lastError = null;
                        for (let attempt = 0; attempt < 2; attempt++) {
                            try {
                                // curl 通道：继承系统代理（Clash 等），稳定不超时；
                                // 404/403 由 -f 转为失败返回 null，不重试。
                                const got = await curlText(`https://raw.githubusercontent.com/${repo}/HEAD/${file}`, 15000);
                                if (got === null) {
                                    lastError = new Error('curl fetch failed');
                                    continue; // 网络错误：重试一次
                                }
                                const { text, repaired } = decodeReadme(got.buf);
                                return { file, text, repaired };
                            }
                            catch (error) {
                                if (attempt === 0) {
                    ...
                    // 指定路径直取：README 内容里的语言链接点击（如「中文」→
                    // README.zh.md），在弹窗内重新加载该文件，语言由内容检测。
                    if (explicitPath !== undefined && explicitPath !== '') {
                        const pkey = `path:${repo}\u0000${explicitPath}`;
                        if (fresh !== true) {
                            const cachedP = readmeCacheGet(pkey);
                            if (cachedP !== undefined) {
                                return cachedP;
                            }
                        }
                        const linked = await fetchReadmeFile(explicitPath);
                        if (linked === null) return null;
                        const fileDir = linked.file.includes('/') ? linked.file.slice(0, linked.file.lastIndexOf('/')) : '';
                        const payload = {
                            ok: true,
                            name: repo,
                            repo,
                            content: rewriteReadmeUrls(linked.text, repo, fileDir),
                            source: linked.file,
                            contentLang: readmeLangOf(linked.file, linked.text),
                            repaired: linked.repaired,
                            remote: true,
                        };
                        readmeCacheSet(pkey, payload);
                        return payload;
                    }
                    const pickReadme = async (files) => {
                        const settled = await Promise.allSettled(files.map(fetchReadmeFile));
                        const hit = settled.find(r => r.status === 'fulfilled' && r.value !== null);
                        return hit === undefined ? null : hit.value;
                    };
                    // 把 README 里的语言切换链接解析成仓库内 raw 相对路径；支持
                    // 相对路径、./ 前缀、/ 根路径、github blob 与 raw 绝对链接。
                    const resolveRawPath = (href) => {
                        let h = String(href || '').split('#')[0].trim();
                        if (h === '' || h.includes('..')) return null;
                        h = decodeURIComponent(h);
                        if (/^https?:\/\//i.test(h)) {
                            let m = /^https?:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+?\/blob\/[^/]+\/(.+)$/i.exec(h);
                            if (m) return m[1];
                            m = /^https?:\/\/raw\.githubusercontent\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+?\/[^/]+\/(.+)$/i.exec(h);
                            if (m) return m[1];
                            return null; // 外部链接不跟随
                        }
                        h = h.replace(/^\.\//, '');
                    ...
                    const findLangLink = (text, wantLang) => {
                        const re = wantLang === 'zh'
                            ? /\[[^\]]*(?:中文|Chinese|简体|繁體|繁体)[^\]]*\]\(([^)\s]+)\)/gi
                            : /\[[^\]]*English[^\]]*\]\(([^)\s]+)\)/gi;
                        let m;
                        while ((m = re.exec(text)) !== null) {
                            const path = resolveRawPath(m[1]);
                            if (path !== null) return path;
                        }
                        return null;
                    };
                    // 主候选 + 罕见命名兜底，语言不符时跟随 README 内语言链接。
                    let found = await pickReadme(wantLang === 'zh' ? ['README.zh.md', 'README.md'] : ['README.md', 'README.zh.md']);
                    if (found === null) found = await pickReadme(['README.zh-CN.md', 'README_zh.md', 'README']);
                    if (found !== null && readmeLangOf(found.file, found.text) !== wantLang) {
                        const linkPath = findLangLink(found.text, wantLang);
                        if (linkPath !== null && linkPath !== found.file) {
                            const linked = await fetchReadmeFile(linkPath);
                            if (linked !== null && readmeLangOf(linked.file, linked.text) === wantLang) found = linked;
                        }
                    }
                    if (found === null) return null;
                    const { file, text, repaired } = found;
                    const contentLang = readmeLangOf(file, text);
                    const fileDir = file.includes('/') ? file.slice(0, file.lastIndexOf('/')) : '';
                    const payload = { ok: true, name: repo, repo, content: rewriteReadmeUrls(text, repo, fileDir), source: file, contentLang, repaired, remote: true };
                    readmeCacheSet(cacheKey, payload);
                    return payload;
                };
                // 已安装插件 → GitHub 仓库地址：install spec 的 github: 前缀 /
                // package.json repository / 本地 README 里的 github 链接。
                const resolveInstalledRepo = (pkgName, spec) => {
                    if (typeof spec === 'string' && /^github:/.test(spec)) {
                        const r = spec.slice(7).replace(/\.git$/, '').replace(/\/+$/, '');
                    ...
                    for (const dir of [join(profileDir(config.profile), 'node_modules', pkgName), join(profileDir(config.profile), '..', 'node_modules', pkgName)]) {
                        try {
                            ...
                            const repoRef = typeof repoField === 'string' ? repoField : (repoField && typeof repoField.url === 'string' ? repoField.url : '');
                            let m = /github\.com[\/:]([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+?)(?:\.git)?$/.exec(repoRef);
                            if (m) return m[1];
                            if (/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repoRef)) return repoRef; // 如 "owner/repo"
                        }
                        catch { /* 无 manifest */ }
                        // 兜底：本地 README 里提到自己的 github 链接（如 dsh-plugin-audit）。
                        try {
                            const found = scanReadmeForRepo(dir, pkgName);
                ...
                // 发现页的社区插件：按 GitHub 仓库地址拉取（链接返回的内容）。
                if (repoUrl !== '') {
                    ...
                    // fresh=1：跳过缓存强制重拉（弹窗「刷新」按钮用）。
                    const fresh = u.searchParams.get('fresh') === '1';
                    // 指定文件直取：README 内语言链接点击后在弹窗内重载该文件。
                    const pathParam = u.searchParams.get('path') ?? '';
                    if (pathParam !== '') {
                        let p;
                        try { p = decodeURIComponent(pathParam); }
                        catch { p = ''; }
                        if (!/^[A-Za-z0-9_./-]+\.(md|markdown)$/i.test(p) || p.includes('..') || p.startsWith('/')) {
                            sendJson(response, 400, { error: 'invalid path' });
                            return;
                        }
                        const payload = await fetchRemoteReadme(repo, lang, p, fresh);
                        if (payload !== null) {
                            sendJson(response, 200, { ...payload, name: name || payload.name });
                            return;
                        }
                        sendJson(response, 200, { ok: false, description: '', error: 'README 拉取失败: 文件不可用' });
                        return;
                    }
                    const payload = await fetchRemoteReadme(repo, lang, undefined, fresh);
                    if (payload !== null) {
                        // 后台预热另一语言（不阻塞响应）：用户切 中文/EN 时秒回。
                        fetchRemoteReadme(repo, lang === 'zh' ? 'en' : 'zh').catch(() => {});
                        sendJson(response, 200, { ...payload, name: name || payload.name });
                        return;
                    }
                    sendJson(response, 200, { ok: false, description: '', error: 'README 拉取失败: 候选文件均不可用' });
                    return;
                }
                // 已安装插件：优先 GitHub 在线 README（「链接返回回来的」内容，
                // 与社区插件一致），在线不可用时回退本地 node_modules 文件。
                if (mergedInstalled()[name] === undefined) {
                    sendJson(response, 404, { error: 'unknown plugin' });
                    return;
                }
                const installedRepo = resolveInstalledRepo(name, mergedInstalled()[name]);
                let payload = installedRepo === null ? null : await fetchRemoteReadme(installedRepo, lang);
                if (payload !== null) {
                    // 后台预热另一语言（不阻塞响应）。
                    fetchRemoteReadme(installedRepo, lang === 'zh' ? 'en' : 'zh').catch(() => {});
                }
                if (payload === null) {
                    ...
                    payload = { ok: true, name, ...readme };
                }
                sendJson(response, 200, { ...payload, name });
            }),
        }),
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dsh-plugin-market] update failed: ${sanitize(message)}`);
                ...
                try {
                    ...
                    if (spec === undefined && readLoaderEntries().some((row) => row.name === name && !row.name.startsWith('@deepseek-ai/') && !row.name.startsWith('cordis:'))) {
                        const row = readLoaderEntries().find((r) => r.name === name);
                         try {
                             // 先收集该包声明的装配引用（删 node_modules 前），
                             // 连同 loader row id 清掉 patch 里的历史 insert 行。
                             const bundleRefs = collectBundleRefs(name);
                             cleanProfilePatch(config.profile, name, [row.id, ...bundleRefs.ids], [...bundleRefs.names, name]);
                             ...
                             // 不 dispose/update 运行中的 entry（Include 写盘
                             // 副作用会破坏 patch），只做会话内视图屏蔽。
                             uninstalledNames.add(name);
                             dropBundleEntry(config.profile, name);
logEvent('info', 'uninstall', `${name} (patch row ${row.id}) removed`);
                            sendJson(response, 200, {
                                ok: true,
                                hot: false,
                                needsRestart: true,
                    ...
                    installing = true;
                    try {
                        // remove 前收集该包声明的装配 id/name（node_modules 删除
                        // 后就拿不到了），用于清理 profile cordis.patch.yml 里的
                        // 历史 insert 行（如 usage-status）——不清理会在下次启动时
                        // include 加载已删除的包，导致 harness 启动失败。
                        const bundleRefs = collectBundleRefs(name);
                        ...
                        if (ok) {
                            ...
                            // 运行中 bundle 层装配的 entry 不 dispose/update：
                            // Include 的 dispose 会触发写盘，可能破坏
                            // cordis.patch.yml 导致下次启动失败。只做会话内
                            // 视图屏蔽——重启后依赖已删，自然不再装配。
                            uninstalledNames.add(name);
                            // 从 profile 顶层 bundles 数组移除（防重启后复活装配）。
                            dropBundleEntry(config.profile, name);
                            ...
                            cleanProfilePatch(config.profile, name, [...rowIds, ...bundleRefs.ids], [...bundleRefs.names, name]);
                            forgetTimes(config.profile, name);
                            // dsh remove 不删 node_modules 里的 link/目录（实测残留）：
                            // 手动清掉，保证卸载干净。top-level 直接依赖删除不影响
                            // .pnpm 里的传递依赖实体。
                            for (const dir of [join(profileDir(config.profile), 'node_modules', name), join(profileDir(config.profile), '..', 'node_modules', name)]) {
                                try {
                                    const stat = lstatSync(dir);
                            ...
                            logEvent('info', 'uninstall', `${name} exit=0 live-removed=${String(hot)} patch-refs=${[...bundleRefs.ids].join(',') || '-'}`);
                        }
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dsh-plugin-market] uninstall failed: ${sanitize(message)}`);
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                        ...
                        if (ok) {
                            ...
                            if (added.length > 0) {
                                const results = await Promise.all(
                                    added.map(name => hotMount(host, profileDir(config.profile), name, loader)),
                                );
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    host.logger?.warn(`[dsh-plugin-market] install failed: ${sanitize(message)}`);
