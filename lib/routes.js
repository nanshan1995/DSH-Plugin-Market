    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                const fetchRemoteReadme = async (repo, wantLang) => {
                    ...
                    const payload = { ok: true, name: repo, content: rewriteReadmeUrls(text, repo, fileDir), source: file, contentLang, repaired, remote: true };
                ...
                if (repoUrl !== '') {
                    ...
                    const payload = await fetchRemoteReadme(m[1].replace(/\.git$/, ''), lang);
                    if (payload !== null) {
                        // 后台预热另一语言（不阻塞响应）：用户切 中文/EN 时秒回。
                        fetchRemoteReadme(m[1].replace(/\.git$/, ''), lang === 'zh' ? 'en' : 'zh').catch(() => {});
