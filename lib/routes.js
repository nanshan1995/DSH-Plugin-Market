...
export function mountMarketRoutes(host, config, llm, loader) {
     ...
     */
    function rewriteReadmeUrls(content, repo) {
        const base = `https://raw.githubusercontent.com/${repo}/HEAD/`;
        const ghPage = `https://github.com/${repo}/blob/HEAD/`;
            ...
            handler: async (request, response) => {
                ...
                if (repoUrl !== '') {
                    ...
                    if (cachedReadme !== undefined && Date.now() - cachedReadme.at < README_CACHE_TTL_MS) {
                        sendJson(response, 200, { ...cachedReadme.data });
                        return;
                    }
                    const candidates = lang === 'zh'
                        ? ['README.zh.md', 'README.zh-CN.md', 'README_zh.md', 'README.md', 'README']
                        : ['README.md', 'README.zh.md', 'README.zh-CN.md', 'README_zh.md', 'README'];
                    // 并行发起全部候选（提速，避免串行 404 逐个等网络），
                    // 但按候选顺序取「第一个成功」——Promise.any 会取先完成的，
                    // 并发下 README.md 可能比 README.zh.md 先返回，导致 zh
                    // 请求错拿英文文件；所以用 allSettled 等全部结束后按序选。
                    const settled = await Promise.allSettled(candidates.map(async (file) => {
                    ...
                    if (hit !== undefined && hit.value !== undefined) {
                        const { file, text, repaired } = hit.value;
                        const content = rewriteReadmeUrls(text, repo);
                        const payload = { ok: true, name: name || repo, content, source: file, contentLang: readmeLangOf(file, text), repaired, remote: true };
                        README_CACHE.set(cacheKey, { at: Date.now(), data: payload });
                        sendJson(response, 200, { ...payload });
                        return;
                    }
                    const reasons = settled.map(r => r.status === 'rejected' ? String(r.reason && r.reason.message || r.reason) : '').filter(Boolean);
                    sendJson(response, 200, { ok: false, description: '', error: `README 拉取失败: ${reasons.join('; ') || 'unknown'}` });
                    return;
                }
