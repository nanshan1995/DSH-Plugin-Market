...
function collectTimes(profile, installed) {
    ...
    for (const name of Object.keys(installed)) {
        let mtime = 0;
        try {
            mtime = lstatSync(join(profileDir(profile), 'node_modules', name)).mtimeMs;
        }
        catch { /* linked/missing entry — unknown */ }
...
export async function searchPlugins(query, lang = 'en', limit = 12, page = 1, sort = 'hot', extraTerms = []) {
    ...
    if (hit !== undefined && Date.now() - hit.at < GH_SEARCH_TTL_MS) {
        ...
        // quoted. Browse keeps plain topic listing; `new` ranks by updated.
        const ghTerms = browseAll ? [] : terms.slice(0, 4).map(t => t.includes(' ') ? `"${t}"` : t);
 ...
 */
export function mountMarketRoutes(host, config, llm) {
