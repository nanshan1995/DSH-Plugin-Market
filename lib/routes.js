...
// 文件；条目 1 小时后自动过期释放（读取时惰性检查，无需定时器）。
    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: async (request, response) => {
                ...
                const fetchRemoteReadme = async (repo, wantLang, explicitPath) => {
                    const cacheKey = `${repo}\u0000${wantLang}`;
                    const cachedReadme = README_CACHE.get(cacheKey);
                    if (explicitPath === undefined || explicitPath === '') {
                        if (cachedReadme !== undefined && Date.now() - cachedReadme.at < README_CACHE_TTL_MS) {
                            return cachedReadme.data;
                        }
                    }
                    ...
                    if (explicitPath !== undefined && explicitPath !== '') {
                        ...
                        const cachedP = README_CACHE.get(pkey);
                        if (cachedP !== undefined && Date.now() - cachedP.at < README_CACHE_TTL_MS) {
                            return cachedP.data;
                        }
                        ...
                        README_CACHE.set(pkey, { at: Date.now(), data: payload });
                        return payload;
                    }
                    ...
                    README_CACHE.set(cacheKey, { at: Date.now(), data: payload });
                    return payload;
                };
                ...
                if (repoUrl !== '') {
                    ...
                    if (pathParam !== '') {
                        ...
                        const payload = await fetchRemoteReadme(repo, lang, p);
                        if (payload !== null) {
                    ...
                    const payload = await fetchRemoteReadme(repo, lang);
                    if (payload !== null) {
