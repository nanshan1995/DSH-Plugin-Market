            ...
            handler: async (request, response) => {
                ...
                if (repoUrl !== '') {
                    ...
                    const candidates = lang === 'zh'
                        ? ['README.zh.md', 'README.zh-CN.md', 'README_zh.md', 'README.md', 'README']
                        : ['README.md', 'README.zh.md', 'README.zh-CN.md', 'README_zh.md', 'README'];
                    // 并行拉取所有候选，第一个成功的立即返回（Promise.any），
                    // 避免串行 404 逐个等满超时。
                    try {
                        const hit = await Promise.any(candidates.map(async (file) => {
                        ...
                        const content = rewriteReadmeUrls(hit.text, repo);
                        const payload = { ok: true, name: name || repo, content, source: hit.file, contentLang: readmeLangOf(hit.file), repaired: hit.repaired, remote: true };
                        README_CACHE.set(cacheKey, { at: Date.now(), data: payload });
                        sendJson(response, 200, { ...payload });
                        return;
                    }
                    catch (error) {
                        const reasons = Array.isArray(error && error.errors) ? error.errors.map(e => String(e && e.message || e)) : [String(error && error.message || error)];
