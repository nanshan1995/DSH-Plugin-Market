                    ...
                    if (spec === undefined && readLoaderEntries().some((row) => row.name === name && !row.name.startsWith('@deepseek-ai/') && !row.name.startsWith('cordis:'))) {
                        const row = readLoaderEntries().find((r) => r.name === name);
                         try {
                             ...
                             // 停掉运行中的残留 entry，并屏蔽管理视图。
                             for (const cand of loader.entries()) {
                                 if (cand.options?.name === name || cand.options?.id === name) {
                                     try {
                                         if (typeof cand.dispose === 'function') await cand.dispose();
                                         else await cand.update({ disabled: true });
                                     }
                                     catch { /* 残留行由 uninstalledNames 屏蔽 */ }
                                 }
                             }
...
logEvent('info', 'uninstall', `${name} (patch row ${row.id}) removed`);
                    ...
                    try {
                        ...
                        if (ok) {
                            ...
                            // 运行中的 bundle 层 entry（include 装配）不会因 dsh
                            // remove 自动移除：先取 rowIds（清理 patch 用），再停掉
                            // 这些 entry，并把 name 记入本会话卸载集合（视图屏蔽，
                            // 防止 UI 显示「已安装」残留）。
                            ...
                            for (const cand of loader.entries()) {
                                if (cand.options?.name === name || cand.options?.id === name) {
                                    try {
                                        if (typeof cand.dispose === 'function') {
                                            await cand.dispose();
                                        }
                                        else {
                                            await cand.update({ disabled: true });
                                        }
                                        liveStopped = true;
                                    }
                                    catch { /* 残留行由 uninstalledNames 屏蔽 */ }
                            ...
                            forgetTimes(config.profile, name);
                            logEvent('info', 'uninstall', `${name} exit=0 live-removed=${String(hot)} live-stopped=${String(liveStopped)}`);
                        }
