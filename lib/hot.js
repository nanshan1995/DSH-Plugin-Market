...
import { logEvent, sanitize } from './log.js';
const HOT_DIR = '.dsh-plugin-market';
let hotTreeClass;
/**
 * 最通用的 Include 类获取方式：从运行中的 `cordis:include` 装配行的
 * fiber 实例直接拿构造器——与 node/dsh 的安装方式完全无关（nvm/fnm/
 * volta/scoop/homebrew 都适用）。安全校验：类必须带 `static inject` 且
 * 包含 'loader'（Include 的特征），否则拒绝。
 */
function includeClassFromLoader(loader) {
    try {
        for (const entry of loader.entries()) {
            const options = entry.options;
            if (!options || options.name !== 'cordis:include' || entry.fiber === void 0) continue;
            const cls = entry.fiber.constructor;
            if (typeof cls === 'function' && Array.isArray(cls.inject) && cls.inject.includes('loader')) return cls;
        }
    }
...
function includeSpecifierCandidates() {
    ...
    try {
        ...
        const root = execFileSync(npmBin, ['root', '-g'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], timeout: 15000, shell: process.platform === 'win32' }).trim();
 ...
 */
export async function hotMount(ctx, profileDir, packageName, loader) {
    try {
        // 优先从运行中的 include 实例取类（与 node 安装方式无关）；
        // 拿不到再走候选 import；都没有则回退重启。
        let HotTree = null;
        if (loader !== undefined && loader !== null) {
            const LiveInclude = includeClassFromLoader(loader);
            if (LiveInclude !== null) {
                HotTree = class MarketHotTreeFromLive extends LiveInclude {
                    /** Runtime-only mount list; the bundle layer owns persistence. */
                    write() { }
                };
            }
        }
        if (HotTree === null) HotTree = await loadHotTreeClass();
        if (HotTree === null)
            return false;
        ...
        ctx.logger?.warn(`[dsh-plugin-market] hot mount of ${packageName} failed, restart required: ${sanitize(message)}`);
