/**
 * dsh-plugin-market host entry: mounts the market's HTTP routes once the profile
...
export function apply(ctx, config) {
    ...
    ctx.inject(['webServer'], (hostCtx) => {
        hostCtx.effect(() => mountMarketRoutes(hostCtx, resolved, ctx.get('llm'), ctx.get('loader')), 'dsh-plugin-market: http routes');
    });
}
